...
float running(char ex, int dur, int wt, char iL){
    float calorie=0.0;
    if(iL=='L' || iL == 'l') return calorie=(8 * wt * dur) / 60;
    else if(iL=='M' || iL=='m') return calorie=(11 * wt * dur) / 60;
    else if(iL == 'H'|| iL == 'h') return calorie=(14 * wt * dur) / 60;
    else return calorie;
}
float cycling(char ex, int dur, int wt, char iL){
    float calorie=0.0;
    if(iL == 'L' || ex == 'l') return calorie=(6 * wt * dur) / 60;
    else if(iL == 'M' || iL == 'm') return calorie=(9 * wt * dur) / 60;
    else if(iL == 'H'|| iL == 'h') return calorie=(12 * wt * dur) / 60;
    else return calorie;
}
float weightLifting(char ex, int dur, int wt, char iL){
    float calorie=0.0;
    if(iL == 'L' || ex == 'l') return calorie=(3 * wt * dur) / 60;
    else if(iL == 'M'|| iL == 'm') return calorie=(6 * wt * dur) / 60;
    else if(iL == 'H'|| iL == 'h') return calorie=(9 * wt * dur) / 60;
    else return calorie;
}
float swimming(char ex, int dur, int wt, char iL){
    float calorie=0.0;
    if(iL == 'L' || ex == 'l') return calorie=(7 * wt * dur) / 60;
    else if(iL == 'M'|| iL == 'm') return calorie=(10 * wt * dur) / 60;
    else if(iL == 'H'|| iL == 'h') return calorie=(13 * wt * dur) / 60;
    else return calorie;
}
...
void workoutSummary(char ex, int dur, int HR, int wt, char iL){
    ...
    if(ex =='R' || ex =='r'){
        cout<<"Running";
        calorie=running(ex, dur, wt, iL);
    }else if(ex == 'C' || ex =='c'){
        cout<<"Cycling";
        calorie=cycling(ex, dur, wt, iL);
    }else if(ex == 'W'|| ex == 'w'){
        cout<<"Weight Lifting";
        calorie=weightLifting(ex, dur, wt, iL);
    }else if(ex == 'S'|| ex == 's'){
        cout<<"Swimming";
        calorie=swimming(ex, dur, wt, iL);
    }
